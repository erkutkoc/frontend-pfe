<script>
    ...
    $: {
        ...
        promise.then((result) => {
            console.log(result)
            membersArray = result.filter(m => new Date(m.banni).valueOf() < Date.now()).sort((a, b) => a.email.localeCompare(b.email)) 
    ...
    function handleInput(e){
        ...
        filteredMembers = membersArray.filter((m) => m.email.startsWith(emailSearched))
        filtered = true
    }
...
<main>
    ...
    <div class="min-h-full flex items-center justify-center py-12 px-15 sm:px-6 lg:px-8">
        <div class="max-w-4xl w-full space-y-8">
            <div>
                <h2 class="mt-6 text-center text-3xl font-extrabold text-gray-900">Voici l'ensemble des utilisateurs:</h2>
            </div>
            ...
            <div>
                {#if filteredMembers.length != 0}
                    {#each filteredMembers as member}
                        <Member email={member.email} id={member.id} token={token} />
                    {/each}
                {:else if filtered}
