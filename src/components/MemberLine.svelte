<script>
	...
	const ban = (e, duration) => {
		if (duration == 0) e.target.innerText = 'OK';
			...
			if (duration == 1) e.target.innerText = 'Banni pour 1 jour';
			else if (duration == -1) e.target.innerText = 'Banni définitivement';
			else e.target.innerText = "Banni pour "+duration+ " jours";
		}
...
<tr>
	...
	<td style="">
		<div class="is-pulled-right">
			{#if !admin}
				{#if banned}
					<button class="button deep-purple accent-3" disabled>
						Banni jusqu'au {bandDateFormat}</button
					>
					<button on:click={(e) => ban(e, 0)} class="button blue darken-1"
						>Supprimer le ban
					</button>
				{:else}
					...
					<button on:click={(e) => ban(e, -1)} class="button red darken-4"
						>Bannir définitivement
					</button>

					{#if admin}
						<button on:click={(e) => makeAdmin(e)} class="button blue darken-2" disabled
							>Déjà Admin
						</button>
					{:else}
						<button on:click={(e) => makeAdmin(e)} class="button green lighten-2"
